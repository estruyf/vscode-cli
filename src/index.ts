...
import { LocalizationListener } from "./tasks/localization";
...
export async function cli(args: string[]) {
  try {
    ...
    } else if (options.task === "localization") {
      await LocalizationListener.listen(options as Args);
    }
...
async function promptMissingTask(options: any) {
  ...
  if (!options.task) {
    questions.push({
      ...
      choices: ["settings", "commands", "localization", "help"],
    });
  }
